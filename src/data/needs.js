...
// Deliberately tiny: everyone gets `food`; dogs additionally get `bathroom`
// (DESIGN.md's "Cupcake needs to go to the bathroom!" bottom-left call-out).
// Never punishing — timers are long and a need just sits there patiently
// (showing a small indicator) until the player gets to it.
//
...
export function clearNeed(stay, key) {
  ...
  stay.timers[key] = key === 'bathroom' ? BATHROOM_INTERVAL() : FOOD_INTERVAL();
}
